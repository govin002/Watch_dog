...
function requireFile() {
  ...
  function fileTransportFactory(logger, { registry = globalRegistry, externalApi } = {}) {
    ...
    function readAllLogs({ fileFilter = (f) => f.endsWith(".log") } = {}) {
      initializeOnFirstAccess();
      const logsPath2 = path$1.dirname(transport.resolvePathFn(pathVariables));
      if (!fs$1.existsSync(logsPath2)) {
        return [];
      }
      return fs$1.readdirSync(logsPath2).map((fileName) => path$1.join(logsPath2, fileName)).filter(fileFilter).map((logPath) => {
        try {
          return {
            path: logPath,
...
(function(module2) {
  ...
  const windows = async () => {
    const bin = path$1.join(__dirname, "fastlist.exe");
  ...
  const main2 = async (options = {}) => {
    ...
    return Object.entries(ret).filter(([, value]) => value.comm && value.args && value.ppid && value.uid && value["%cpu"] && value["%mem"]).map(([key, value]) => ({
      ...
      name: path$1.basename(value.comm),
      cmd: value.args,
...
var psListExports = psList$1.exports;
const psList = /* @__PURE__ */ getDefaultExportFromCjs(psListExports);
...
const userDataPath = require$$0$4.app.getPath("userData");
const configPath = path__namespace.join(userDataPath, CONFIG_FILE);
const logsPath = path__namespace.join(userDataPath, LOGS_FILE);
let config = { apps: [], interval: 5, logRetentionDays: 1 };
let mainWindow = null;
...
function loadConfig() {
  try {
    if (fs__namespace.existsSync(configPath)) {
      ...
      if (config.logRetentionDays === void 0) config.logRetentionDays = 1;
    } else {
      saveConfig();
    }
  } catch (err) {
...
function addLogEntry(appId, appName, event) {
  try {
    const now = Date.now();
    ...
    if (fs__namespace.existsSync(logsPath)) {
      try {
        logs = JSON.parse(fs__namespace.readFileSync(logsPath, "utf-8"));
      } catch {
        logs = [];
      }
    }
    logs.unshift(entry);
    const retentionMs = config.logRetentionDays * 24 * 60 * 60 * 1e3;
    logs = logs.filter((l) => {
      if (!l.rawTime) return true;
      return now - l.rawTime < retentionMs;
    });
    if (logs.length > 500) logs = logs.slice(0, 500);
    fs__namespace.writeFileSync(logsPath, JSON.stringify(logs, null, 2));
    mainWindow == null ? void 0 : mainWindow.webContents.send("logs-updated", logs);
  } catch (err) {
    log.error("Failed to write log", err);
  }
}
async function isProcessRunning(appPath) {
  const basename = path__namespace.basename(appPath).toLowerCase();
  if (process.platform === "win32") {
    return new Promise((resolve) => {
      require$$0.exec(`tasklist /FO CSV /NH /FI "IMAGENAME eq ${basename}"`, (err, stdout) => {
        if (err) {
          resolve(true);
          return;
        }
...
async function checkApps() {
  ...
  for (const appItem of config.apps) {
    ...
    if (appItem.lastRestartTime && now - appItem.lastRestartTime < 1e4) {
      if (running) {
  ...
  if (changed) {
    mainWindow == null ? void 0 : mainWindow.webContents.send("status-update", config.apps);
  }
  isChecking = false;
}
function startMonitoring() {
  ...
  checkApps();
  monitorTimer = setInterval(checkApps, config.interval * 1e3);
}
...
function pruneLogs() {
  try {
    if (!fs__namespace.existsSync(logsPath)) return;
    const now = Date.now();
    const data = fs__namespace.readFileSync(logsPath, "utf-8");
...
require$$0$4.app.whenReady().then(() => {
  ...
  require$$0$4.app.on("activate", () => {
    if (require$$0$4.BrowserWindow.getAllWindows().length === 0) createWindow();
  });
});
require$$0$4.app.on("window-all-closed", () => {
  if (process.platform !== "darwin") require$$0$4.app.quit();
});
require$$0$4.ipcMain.handle("get-config", () => config);
require$$0$4.ipcMain.handle("get-logs", () => {
  try {
    return fs__namespace.existsSync(logsPath) ? JSON.parse(fs__namespace.readFileSync(logsPath, "utf-8")) : [];
  } catch {
    return [];
  }
});
require$$0$4.ipcMain.handle("browse-file", async () => {
  const result = await require$$0$4.dialog.showOpenDialog(mainWindow, {
    properties: ["openFile"],
    filters: [{ name: "Executables", extensions: ["exe"] }]
  });
  return result.canceled ? null : result.filePaths[0];
});
require$$0$4.ipcMain.handle("add-app", (event, { name, path: appPath }) => {
  const entry = {
...
require$$0$4.ipcMain.handle("remove-app", (event, id) => {
  config.apps = config.apps.filter((a) => a.id !== id);
  saveConfig();
  return true;
});
require$$0$4.ipcMain.handle("toggle-auto-restart", (event, { id, value }) => {
...
require$$0$4.ipcMain.handle("update-interval", (event, interval) => {
  config.interval = interval;
  saveConfig();
  startMonitoring();
  return true;
});
require$$0$4.ipcMain.handle("update-log-retention", (event, days) => {
  config.logRetentionDays = days;
  saveConfig();
  pruneLogs();
  try {
    if (fs__namespace.existsSync(logsPath)) {
      const logs = JSON.parse(fs__namespace.readFileSync(logsPath, "utf-8"));
      mainWindow == null ? void 0 : mainWindow.webContents.send("logs-updated", logs);
    }
  } catch {
  }
...
require$$0$4.ipcMain.handle("open-config-folder", () => {
  require$$0$4.shell.openPath(userDataPath);
});
